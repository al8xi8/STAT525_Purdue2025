...
/**************************************************************************
 * Problem 6: Sales Growth Transformation
 **************************************************************************/
...
proc transreg data=sales;
    model boxcox(sales_growth / convenient lambda=-1 to 1 by 0.25) = identity(time);
run;

/* 6c: Apply sqrt transformation based on Box-Cox */
data sales_transformed;
    set sales;
    sqrt_sales = sqrt(sales_growth);
run;

symbol1 v=circle i=sm50 c=blue;
symbol2 v=none i=rl c=black;

proc gplot data=sales_transformed;
    plot sqrt_sales*time=1 sqrt_sales*time=2 / overlay;
    label sqrt_sales = "Sqrt(Sales Growth)"
          time = "Time (years)";
    title "Scatterplot of Sqrt(Sales Growth) vs. Time with Regression Line";
run;
quit;
